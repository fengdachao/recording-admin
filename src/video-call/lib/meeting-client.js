...
async function startMeeting() {
  ...
  for(var i = 0; i < userList.length; i++) {
    (function(index) {
      setTimeout(function () {
        console.log('invite user index########################:', index)
        invite(userList[index])
      }, timeout += 5000)
    })(i)
  }
...
function handleICEConnectionStateChangeEvent(event) {
  log("*** ICE connection state changed to " + myPeerConnectionList[targetUsername].iceConnectionState);
...
function handleSignalingStateChangeEvent(event) {
  log("*** WebRTC signaling state changed to: " + myPeerConnectionList[targetUsername].signalingState);
...
function handleICEGatheringStateChangeEvent(event) {
  log("*** ICE gathering state changed to: " + myPeerConnectionList[targetUsername].iceGatheringState);
}
...
function handleUserlistMsg(msg) {
  ...
  msg.users.forEach(function(username) {
    ...
    userCheckbox.value = username
    userCheckbox.type = 'checkbox'
    // userCheckbox.innerHTML = username.toString()
    // userCheckbox.onchange = function(e) {
    //   if (e.target.checked) {
    //     invite(username)
    //   } else {
    //     hangUpCall()
    ...
    // onlineUsers.appendChild(userCheckbox)
    // onlineUsers.appendChild(userLabel)
  });
...
function closeVideoCall() {
  ...
  if (myPeerConnection) {
    ...
    console.log('close call transceivers:', myPeerConnection.getTransceivers().length)
...
async function invite(userName) {
  ...
  try {
    webcamStreamMap[targetUsername] = await navigator.mediaDevices.getUserMedia(mediaConstraints);
    document.getElementById("local_video").srcObject = webcamStreamMap[targetUsername]
  } catch(err) {
  ...
  try {
    ...
    webcamStreamMap[targetUsername].getTracks().forEach(
      transceiver = track => myPeerConnectionList[targetUsername].addTransceiver(track, {streams: [webcamStreamMap[targetUsername]]})
    );
  } catch(err) {
    handleGetUserMediaError(err);
  }
...
async function handleVideoOfferMsg(msg) {
  ...
  if (myPeerConnectionList[targetUsername].signalingState != "stable") {
    ...
    // until both return.
    await Promise.all([
      myPeerConnectionList[targetUsername].setLocalDescription({type: "rollback"}),
  ...
  if (!webcamStream) {
    try {
      webcamStream = webcamStreamMap[targetUsername] = await navigator.mediaDevices.getUserMedia(mediaConstraints);
    ...
    try {
      webcamStream.getTracks().forEach(
        transceiver = track => myPeerConnectionList[targetUsername].addTransceiver(track, {streams: [webcamStream]})
      );
    } catch(err) {
      handleGetUserMediaError(err);
    }
  ...
  await myPeerConnectionList[targetUsername].setLocalDescription(await myPeerConnectionList[targetUsername].createAnswer());
...
async function handleVideoAnswerMsg(msg) {
  ...
  var desc = new RTCSessionDescription(msg.sdp);
  await myPeerConnectionList[targetUsername].setRemoteDescription(desc).catch(reportError);
